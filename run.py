...
cnn = torch.load(model_address, map_location=device)
...
for record in SeqIO.parse("../../common/RNA_section__454_9627.fasta", "fasta"):
    seq = record.seq

def seq_to_representation(seq, onehot=True):
    return create_matrix(seq, onehot=onehot)

def input_tensor_to_target_matrix(tensor):
    return cnn(tensor)

def get_pairing_pos(row):
    pair_pos = None
...
def target_matrix_to_vienna(tensor):
    vienna = []

    for i, row in enumerate(tensor.squeeze()):
        pair_pos = -1
        for j, e in enumerate(row):
            if e == 1:
                pair_pos = j
...
x = seq_to_representation(seq, onehot=True)
...
result_pairings = target_matrix_to_vienna(y_hat_test)
result_vienna = "".join(result_pairings)
print(result_vienna)
...
# x, y, family = next(iter(test_loader))
